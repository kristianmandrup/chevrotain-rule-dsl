...
function toTokenMap(value) {
    if (Array.isArray(value)) {
        return value.reduce(function (acc, item) {
            var name = typeof item === 'function' ? item.name : new String(item);
            acc[name] = item;
        }, {});
    }
...
var RuleParser = (function () {
    function RuleParser(parser, options) {
        if (options === void 0) { options = { logging: false, registry: null, tokensMap: {} }; }
        this.usedRules = {};
        this._registry = {};
        this.code = [];
        this.tokenMap = {};
    ...
    RuleParser.prototype.findToken = function (value) {
        return this.tokenMap[value] || this.tokenMap[value.name];
    };
    RuleParser.prototype.resolveToken = function (token) {
    ...
    RuleParser.prototype.parse = function (rule, options) {
        ...
        if (typeof rule === 'string') {
            ...
            if (/\S+\s+\S+/.test(rule)) {
                ...
                var orExp = /\sor\s/;
                var pipeExp = /\|/;
                var or = orExp.test(rule);
                var orPipe = pipeExp.test(rule);
                // if separator is either 'or' OR '|'
                if (or || orPipe) {
                    var splitExp = or ? orExp : pipeExp;
                    this.log('or rule: split', splitExp);
                    var alternatives = rule.split(splitExp).map(function (alt) { return alt.trim(); });
                    return this.or(alternatives);
                }
                else {
                    this.log('split rules');
                    var list = rule.split(/\s+/).map(function (item) { return item.trim(); });
    ...
    RuleParser.prototype.parseList = function (rules, options) {
        ...
        var alt = isAlt(options);
        var codeJoin = alt ? ',' : '\n';
        var parser = alt ? 'alt' : 'parse';
        var parsedRules = rules.map(function (rule) { return _this[parser](rule, options); });
        var codeStmts = parsedRules.map(function (pr) { return pr.code; }).join(codeJoin);
    ...
    Object.defineProperty(RuleParser.prototype, "islogging", {
        get: function () {
    ...
    RuleParser.prototype.subrule = function (value, options) {
        var _this = this;
        if (options === void 0) { options = {}; }
        var fun = 'SUBRULE';
    ...
    // must be a Token
    RuleParser.prototype.consume = function (value, options) {
        if (options === void 0) { options = {}; }
        this.log('consume', value);
        var token = this.resolveToken(value);
        var code = '$.CONSUME(' + codeOf(token) + ')';
        var $ = this.$;
        var rule = function () { return $.CONSUME(token).bind($); };
    ...
    RuleParser.prototype.or = function (alternatives) {
        ...
        var parsed = this.parse(alternatives, { parent: 'or' });
